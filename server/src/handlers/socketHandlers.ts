...
export function registerSocketHandlers(io: SgmServer, socket: SgmSocket) {
  ...
  // Atualização da Fila de Iniciativa
  ...
  // Atualização de Rodada / Turno da Campanha
  ...
  socket.on('map:ping', ({ x, y }) => {
    ...
    const ping: RoomPing = {
      ...
      x,
      y,
      senderName: member?.name || 'Jogador',
  ...
  function handleLeave() {
    ...
    if (result) {
      ...
      socket.to(room.code).emit('room:member-left', { memberId });
      io.to(room.code).emit('room:members-updated', {
        members: Array.from(room.members.values()),
      });
