...
const PRESET_CONDITIONS: Condition[] = [
  {
    name: 'Fora de Combate',
    desc: 'Sempre pulado na iniciativa.',
    color: 'gray',
    type: 'out_of_combat',
  },
  {
    name: 'Atordoado',
    desc: 'Pula a vez, não pode fazer ações.',
    color: 'yellow',
    type: 'skip_turn',
    durationTurns: 1,
  },
  ...
  {
    name: 'Agarrado',
    desc: 'Desprevenido, imóvel, –1d20 ataque.',
    color: 'red',
  },
  ...
  {
    name: 'Caído',
    desc: 'Deslocamento reduzido, –2d20 ataque corpo a corpo.',
    color: 'yellow',
  },
  {
    name: 'Cego',
    desc: 'Desprevenido e lento. –2d20 Agi/For.',
    color: 'yellow',
  },
  ...
  {
    name: 'Enredado',
    desc: 'Lento, vulnerável, –1d20 ataque.',
    color: 'yellow',
  },
  ...
  {
    name: 'Fascinado',
    desc: 'Não faz ações além de observar.',
    color: 'purple',
    type: 'skip_turn',
  },
  ...
  {
    name: 'Morrendo',
    desc: '0 PV, inconsciente, morre em 3 rodadas.',
    color: 'red',
    type: 'skip_turn',
    durationTurns: 3,
  },
  ...
  {
    name: 'Paralisado',
    desc: 'Imóvel, indefeso, apenas ações mentais.',
    color: 'red',
    type: 'skip_turn',
  },
  {
    name: 'Pasmo',
    desc: 'Não pode fazer ações.',
    color: 'purple',
    type: 'skip_turn',
    durationTurns: 1,
  },
  {
    name: 'Petrificado',
    desc: 'Inconsciente, Resistência Dano 10.',
    color: 'yellow',
    type: 'skip_turn',
  },
  ...
  {
    name: 'Surdo',
    desc: '–2d20 Iniciativa e ruim p/ rituais.',
    color: 'yellow',
  },
  {
    name: 'Surpreendido',
    desc: 'Desprevenido e não pode agir.',
    color: 'yellow',
    type: 'skip_turn',
    durationTurns: 1,
  },
...
export default function ConditionManager({
  ...
  const handleAddCustom = () => {
    ...
    const durationTurns = customDuration
  ...
  const getTypeIcon = (type?: string) => {
    switch (type) {
      case 'skip_turn':
        return (
          <span title="Pula a Vez">
            <Ban className="w-3 h-3 text-red-400" />
          </span>
        );
      case 'out_of_combat':
        return (
          <span title="Fora de Combate">
            <Skull className="w-3 h-3 text-gray-400" />
          </span>
        );
      case 'stat_modifier':
        return (
          <span title="Modificador de Status">
    ...
    <div className="space-y-4">
          ...
          >
            <div className="flex-1 min-w-0 pr-2">
              <div className="flex items-center gap-2">
                <strong className="text-sm text-[#e1e1e6] truncate">
                  {c.name}
                </strong>
                {getTypeIcon(c.type)}
                {c.durationTurns !== undefined && (
                  <span
                    className="flex items-center gap-1 text-[10px] bg-[#323238] text-[#a8a8b3] px-1.5 py-0.5 rounded-full"
              ...
              <span
                className="text-[11px] text-[#a8a8b3] block truncate"
                title={c.desc}
              >
                {c.desc}
              </span>
            ...
            <button
              className="text-[#a8a8b3] hover:text-[#e55757] shrink-0"
          ...
          <SelectContent className="bg-[#202024] border-[#323238] text-[#e1e1e6] max-h-[300px]">
            {PRESET_CONDITIONS.map((p) => (
              <SelectItem
                key={p.name}
                value={p.name}
                className="hover:bg-[#8257e5] cursor-pointer text-sm"
              >
                <div className="flex items-center gap-2">
                  <div
                    className="w-2 h-2 rounded-full"
                    style={{ backgroundColor: colorMap[p.color] }}
                  />
                  {p.name}
                </div>
              </SelectItem>
          ...
          onClick={() => {
            if (selectedPreset) {
              ...
              if (preset) {
                ...
                addCondition({
                  ...preset,
                  durationTurns: isNaN(durationTurns as number)
                    ? undefined
                    : durationTurns,
                });
                setSelectedPreset('');
      ...
      <div className="bg-[#121214] p-2 rounded-lg border border-[#323238] flex flex-col gap-2">
        ...
        <div className="flex gap-2 items-center">
          <Select
            value={customType}
            onValueChange={(val) => setCustomType(val || 'normal')}
          >
          ...
          <Button
            ...
            size="sm"
            variant="outline"
            className="border-[#323238] bg-[#202024] text-[#e1e1e6] hover:bg-white/5 hover:text-white shrink-0 h-8"
          >
            Adicionar Custom
          </Button>
        </div>
