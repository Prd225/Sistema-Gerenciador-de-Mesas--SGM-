...
- **Git** (GitHub Flow, decisão 0008): trabalhe numa branch curta criada a partir da `next` (`feat/`, `fix/`, `refactor/`, `chore/`, `docs/`), com Conventional Commits. **Nunca mexa na `master`**: é a versão do Pedro. Sem push nem PR sem pedido. Não altere `LICENSE` nem o campo `license`. Sem emojis em código, commits e docs.
