  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-[#202024] border-[#323238] text-[#e1e1e6] sm:max-w-[400px]">
        ...
        <p className="text-sm text-[#a8a8b3] mb-4">Insira o valor da iniciativa (ex: 15.02). Deixe em branco para ignorar/remover.</p>
          ...
          {tokens.map(token => (
            <div key={token.id} className="flex items-center justify-between gap-4 p-2 bg-black/20 rounded border border-[#323238]">
              ...
              <Input
                ...
                onChange={(e) => setLocalValues({ ...localValues, [token.id]: e.target.value })}
                className="w-20 bg-[#121214] border-[#323238] h-8 text-center text-[#e1e1e6]"
              />
            </div>
          ))}
          {tokens.length === 0 && <span className="text-sm text-[#a8a8b3]">Crie tokens primeiro.</span>}
        </div>
        ...
        <div className="flex justify-between mt-6">
          ...
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => onOpenChange(false)} className="border-[#323238] bg-transparent text-[#e1e1e6] hover:bg-white/5">Cancelar</Button>
            <Button onClick={handleApply} className="bg-[#8257e5] hover:bg-[#9466ff] text-white">Aplicar</Button>
          </div>
        </div>
      </DialogContent>
    </Dialog>
