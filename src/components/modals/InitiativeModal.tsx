...
function SortableTokenRow({ item, token, onValueChange, sortMode }: { item: any, token: any, onValueChange: (val: string) => void, sortMode: string }) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: item.tokenId });
  ...
  return (
    <div ref={setNodeRef} style={style} className="flex items-center justify-between gap-4 p-2 bg-[#202024] rounded border border-[#323238] shadow-sm mb-2">
      <div className="flex items-center gap-2 flex-1">
        ...
        <span className="font-bold text-sm truncate flex-1">{token?.fullName || 'Desconhecido'}</span>
      </div>
      <Input
        type="number"
        step="0.01"
...
export default function InitiativeModal({ open, onOpenChange }: InitiativeModalProps) {
  ...
  // Tokens not in queue
  const unqueuedTokens = tokens.filter(t => !queue.some(q => q.tokenId === t.id));
        ...
        <div className="max-h-[350px] overflow-y-auto pr-2">
          {queue.length > 0 ? (
            <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
              <SortableContext items={queue.map(q => q.tokenId)} strategy={verticalListSortingStrategy}>
                {queue.map(item => {
                  ...
                  return (
                    <SortableTokenRow 
                      ...
                      token={token} 
                      sortMode={sortMode}
                      onValueChange={(val) => handleValueChange(item.tokenId, val)} 
                    />
                  );
                })}
              </SortableContext>
          ...
          {unqueuedTokens.length > 0 && (
            <div className="mt-4 pt-4 border-t border-[#323238]">
              <span className="text-xs font-semibold uppercase tracking-wider text-[#a8a8b3] mb-2 block">Fora da Fila</span>
              <div className="space-y-2">
                {unqueuedTokens.map(token => (
                  <div key={token.id} className="flex items-center justify-between gap-4 p-2 bg-black/20 rounded border border-[#323238] opacity-60 hover:opacity-100 transition-opacity">
