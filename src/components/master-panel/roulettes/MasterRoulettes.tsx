...
export default function MasterRoulettes() {
  ...
  const handleNextPage = () => setActivePageIndex(p => Math.min(pages.length - 1, p + 1));
  const handleAddPage = () => {
    addPage(`Página ${pages.length + 1}`);
    setActivePageIndex(pages.length);
  };
  const handleRemovePage = () => {
    if (pages.length > 1 && window.confirm('Deseja mesmo apagar esta página e todas as suas roletas?')) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#09090b] relative" ref={containerRef}>
