...
import { BookOpen, Scale, Clapperboard, Music, Table, Dices, NotebookPen } from 'lucide-react';

const PANEL_DETAILS: Record<SubPanelId, { title: string; icon: React.ElementType }> = {
...
export default function SubPanelPlaceholder({ panelId }: { panelId: SubPanelId }) {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center bg-[#121214]/80 border border-[#323238] rounded-xl overflow-hidden shadow-xl p-4 transition-all hover:border-[#8257e5]/50">
      ...
      <h3 className="text-xl font-semibold text-[#e1e1e6] mb-2">{details.title}</h3>
