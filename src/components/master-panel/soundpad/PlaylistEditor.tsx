...
export default function PlaylistEditor({
  ...
  const updatePlaylistSongs = useSoundpadStore(
    (state) => state.updatePlaylistSongs,
  );
  ...
  const handleAddTag = () => {
    if (tagInput.trim()) {
      const newTags = tagInput
        .split(',')
        .map((t) => t.trim())
        .filter((t) => t && !playlist.tags.includes(t));
      if (newTags.length > 0) {
        updatePlaylist(pageId, playlistId, {
  ...
  const handleRemoveTag = (tagToRemove: string) => {
    updatePlaylist(pageId, playlistId, {
      tags: playlist.tags.filter((t) => t !== tagToRemove),
    });
  };
  ...
  const SourceIcon = ({ type }: { type: SongSource }) => {
    ...
    return (
      <span title="Local">
        <HardDrive className="w-3.5 h-3.5 text-[#8257e5]" />
      </span>
    );
  };
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-[#121214]">
      ...
      <div className="border-t border-[#323238] bg-[#1a1a1e] shrink-0 flex flex-col transition-all">
        ...
        >
          <span className="text-xs font-semibold uppercase text-[#7a7a80] px-1">
            Detalhes da Playlist
          </span>
          {isFooterMinimized ? (
            <ChevronUp className="w-4 h-4 text-[#7a7a80]" />
          ) : (
            <ChevronDown className="w-4 h-4 text-[#7a7a80]" />
          )}
        </button>
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="bg-[#202024] hover:bg-[#323238] border border-[#323238] px-3 rounded text-[#a8a8b3] transition-colors"
                >
                  <Plus className="w-4 h-4" />
                </button>
              </div>
              <div className="flex flex-wrap gap-1.5 mt-1 max-h-24 overflow-y-auto custom-scrollbar">
                {playlist.tags.map((tag) => (
                  <span
                    key={tag}
                    className="flex items-center gap-1 text-xs bg-[#202024] border border-[#323238] text-[#a8a8b3] px-2 py-0.5 rounded-full group"
                  >
