      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div
          ...
          className={`flex items-center justify-center w-10 h-10 rounded-full bg-[#121214] border transition-colors cursor-pointer ${
            isActive
              ? 'border-[#1DB954] shadow-[0_0_10px_rgba(29,185,84,0.1)]'
              : 'border-[#323238] hover:border-[#1DB954] group-hover:border-[#8257e5]/50'
          }`}
          ...
          <Music
            className={`w-5 h-5 transition-colors ${
              isActive
                ...
                : 'text-[#8257e5] opacity-80 hover:text-[#1DB954] hover:opacity-100 group-hover:opacity-100'
            }`}
          />
        </div>
      </div>
        ...
        {playlist.tags && playlist.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-1">
            {playlist.tags.map((tag) => (
              <span
                key={tag}
                className="flex items-center gap-0.5 text-[0.65rem] uppercase tracking-wider font-semibold text-[#8257e5] bg-[#8257e5]/10 px-1.5 py-0.5 rounded"
              >
