...
export default function AddMusicModal({ open, onOpenChange, pageId, playlistId }: AddMusicModalProps) {
  ...
  const addSongToPlaylist = useSoundpadStore(state => state.addSongToPlaylist); 
  ...
  const handleAddYoutube = async () => {
    ...
    try {
      ...
      let videoId = '';
      const ytMatch = youtubeLink.match(/(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=)|youtu\.be\/)([^"&?\/\s]{11})/i);
      ...
      <div className="bg-[#121214] border border-[#323238] rounded-xl text-[#e1e1e6] w-full max-w-md overflow-hidden shadow-2xl flex flex-col animate-in zoom-in-95 duration-200">
        ...
        <div className="p-6 min-h-[200px] flex flex-col justify-center">
          ...
          {activeTab === 'youtube' && (
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold text-[#a8a8b3] uppercase tracking-wider">Link do YouTube</label>
                <div className="relative">
                  <input
                    value={youtubeLink}
