...
export default function SoundpadPagination({
  currentPage,
  setCurrentPage,
}: {
  currentPage: number;
  ...
  const handleRemovePage = () => {
    ...
    if (
      confirm(
        'Deseja excluir esta página de playlists? As playlists dentro dela serão perdidas.',
      )
    ) {
        ...
        {isEditing === pages[currentPage].id ? (
          ...
          <div className="flex items-center justify-center gap-2 group w-full">
            ...
            <button
              ...
              className="opacity-0 group-hover:opacity-100 text-[#a8a8b3] hover:text-[#8257e5] transition-all"
            >
