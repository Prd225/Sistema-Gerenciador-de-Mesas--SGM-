...
export const useRoulettesStore = create<RoulettesState>((set) => ({
  ...
  addRoulette: (pageId, title = '', color) =>
    set((state) => {
  ...
  updateRoulette: (pageId, rouletteId, updates) =>
    set((state) => {
      const newState = {
        pages: state.pages.map((p) => {
          if (p.id === pageId) {
  ...
  removeRoulette: (pageId, rouletteId) =>
    set((state) => {
      const newState = {
        pages: state.pages.map((p) => {
          if (p.id === pageId) {
            return {
              ...p,
              roulettes: p.roulettes.filter((r) => r.id !== rouletteId),
            };
          }
          return p;
        }),
